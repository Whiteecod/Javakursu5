...
public class _01_StringBufferAndStringBuilder {
    public static void main(String[] args) {
        ...
        cumle = cumle + "Bugün"; // yani + işareti Stringler de birleştirme yapar
        cumle = cumle + " Hava"; // atama ister
        cumle = cumle + " Çok";
        cumle += " Soğuk";

        System.out.println("cumle = " + cumle);
        System.out.println("cumle = " + cumle.concat(" Fakat dün sıcaktı "));
        System.out.println("cumle = " + cumle);

        cumle = cumle.concat("fakat dün sıcaktı"); // concat de birleştirir, fakat atama ister
        System.out.println("cumle = " + cumle);

        /****************** StringBuilder ******************/
        StringBuilder cumle2 = new StringBuilder();
        cumle2.append("Bugün"); // append : ekle demek atama gerektirmez, direk ekler
        cumle2.append(" Hava");
        cumle2.append(" Çok güzel");
...
//------------------------------------------------------------------------------------------//
        ...
        sb.delete(0, 5); // (index)0 dahil , 5 hariç olmak üzeri siler.
        System.out.println("sb = " + sb);

        sb.deleteCharAt(3); // sadece belirtilen index de ki karakteri siler
        System.out.println("sb = " + sb);

        sb.insert(5, "Kelime"); // 5 nolu index e kelime yi ekle: araya ekleme
        System.out.println("sbn = " + sb);

        sb = new StringBuilder("Bugün hava çok soğuk"); // sıfırlandı ve ilk değer atandı
        System.out.println("sb = " + sb);

        sb.replace(3, 8, "bu"); // verilen aralığı bu strigle değiştirir
        System.out.println("sb = " + sb);
